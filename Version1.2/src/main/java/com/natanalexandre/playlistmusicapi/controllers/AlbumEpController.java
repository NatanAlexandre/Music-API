...
public class AlbumEpController {
    ...
    public AlbumEpController(AlbumEpService albumEpService, ArtistService artistService, MusicService musicService, BandService bandService){
    ...
    public ResponseEntity<Object> saveAlbumEp(@RequestBody @Valid AlbumEpDto albumEpDto){
        if (!artistService.existsByArtistName(albumEpDto.getArtistName()) && !bandService.existsByBandName(albumEpDto.getArtistName())){
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Artist or Band " + albumEpDto.getArtistName() + " not found!");
        }
